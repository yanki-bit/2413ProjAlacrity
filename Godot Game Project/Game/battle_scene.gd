...
func add_enemies( mainEnemy, minion, numberOfMinions ):
	...
	$BattleSceneContainer/EnemiesContainer/MiddleEnemy/MiddleEnemyControl.add_child(enemy1)
	...
	match numberOfMinions:
		2:
			...
			$BattleSceneContainer/EnemiesContainer/MiddleEnemy/LeftEnemyControl.add_child(enemy2)
			enemy2.show_in_fight()
			
			# Add a minion to the right
			enemy3 = load(minion[1]).instantiate()
			enemy3.initialize_stats_in_combat()
			$BattleSceneContainer/EnemiesContainer/MiddleEnemy/RightEnemyControl.add_child(enemy3)
