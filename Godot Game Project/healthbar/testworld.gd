...
func enemy_turn():
	display_text("The enemy retaliates and attacks back!")
	display_text("U IN DEEP SHIT NOW")
	#calculate damage taken
	currentHealth = max(0, currentHealth - enemyDamage)
	
	#update player health
	set_health($Panel/PlayerData/healthBarv3, currentHealth, max_health)
	
	#now it's time to call for the enemy to attack
	display_text("The foe dealt %d damage. OW!" % [enemyDamage]) 
...
func _on_attack_pressed():
	...
	#show how much damage dealt
	display_text("You dealt %d damage! Nice." % [playerDamage]) 
